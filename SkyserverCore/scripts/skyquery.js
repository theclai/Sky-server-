    ...
    $.each(text, function (val1, text1) {
      ...
      $.each(text1, function (val2, text2) {
        ...
        if (text2.output != undefined) {
          ...
          dnlink = "<a href=" + html_link + "  download><span> " + "html" + "</span></a>" + "<a href=\"javascript:void(0)\" onclick =\"downloadTable('" + dbtb[0] + "','" + dbtb[1] + "','csv')\"><span> " + "csv" + "</span></a>" + "<a href=\"javascript:void(0)\" onclick =\"downloadTable('" + dbtb[0] + "','" + dbtb[1] + "','json')\"><span> " + "txt" + "</span></a>"; //"<a href=\"javascript:void(0)\" onclick =\"downloadTable('" + dbtb[0] + "','" + dbtb[1] + "','fits')\"><span> " + "FITS" + "</span></a>" +
        ...
        var buttons = '<button type="button" id="expandQueryButton_' + jobId + '" class="btn btn-sm btn-default" onclick="expandQuery(\'' + jobId + '\')" >Expand Query</button><button type="button" id="reloadQueryButton" class="btn btn-sm btn-default" onclick="reloadQuery(\'' + jobId + '\')" >Reload Query</button>';
        rows.push({
          0: dateCreated,
          1: dateStarted,
          2: dateFinished,
...
function getSqlQuery(queryIndex) {
  ...
  if (crossMatchQuery[queryIndex] != undefined) document.getElementById("crossMatchQuery").value = crossMatchQuery[queryIndex];else document.getElementById("crossMatchQuery").value = "";
}
